...
function build_tree(nodes, options) {
  ...
  _.each(nodes, function (node, id) {
    ...
    if (-1 !== idx) {
      node.id_prefix = node.id.substring(0, idx + 1);
      node.id_suffix = node.id.substring(idx + 1);
    }
...
/**
 *  NDoc.parse(files, options, callback) -> Void
...
NDoc.parse = function parse(files, options, callback) {
  parse_files(files, options, function (err, nodes) {
    callback(err, build_tree(nodes, options));
  });
};
