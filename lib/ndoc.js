...
Fs.readdirSync(__dirname + '/ndoc/plugins').forEach(function (type) {
  Fs.readdirSync(__dirname + '/ndoc/plugins/' + type).forEach(function (plugin) {
    if (/\.js$/.test(plugin)) {
      NDoc.use(require(__dirname + '/ndoc/plugins/' + type + '/' + plugin));
    }
  });
});
