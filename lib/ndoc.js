...
// internal
var interpolate = require('./ndoc/common').interpolate;
...
function preprocess_nodes(nodes, options) {
  _.each(nodes, function (node) {
    if (!node.href && options.linkFormat) {
      node.href = options.linkFormat
        .replace(/\{file\}/g, node.file.replace(/\\/g, '/'))
        .replace(/\{line\}/g, node.line);
      node.href = interpolate(node.href, options);
    }
  });
}


function build_ast(nodes) {
...
NDoc.parse = function parse(files, options, callback) {
  parse_files(files, options, function (err, nodes) {
    nodes = preprocess_nodes(nodes, options);
    callback(err, build_ast(nodes));
  });
};
