...
try {
  ...
  write(__dirname + '/parser.js', (new Jison.Parser(grammar)).generate());
  })();
  var parser = require('./parser');
}
...
function NDoc(files, options) {
  ...
  files.forEach(function(file) {
    ...
    for (var id in ndocs) {
      ndocs[id].href = options.href + '/' + file + ndocs[id].href;
...
NDoc.prototype.search_index = function() {
  var index = {};
...
NDoc.prototype.toHTML = function(options) {
  ...
  var htmlize = require('marked');
  var hilite = require('highlight').Highlight;
  ...
  var vars = {
    title: options.title || '',
    version: '0.0.1', //program.version, // TODO: clashes to program.version()...
    readme: options.index_page && Util.read(options.index_page),
    sections: [],
    tree: tree,
    elements: JSON.stringify(this.search_index()),
    src_code_text: options.src_code_text,
    src_code_href: options.src_code_href,
    path: '/',
    htmlize: htmlize,
    hilite: hilite,
