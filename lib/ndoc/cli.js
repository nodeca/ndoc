...
/**
 *  NDoc.cli.findFiles(paths, excludes, callback(err, files)) -> Void
...
cli.findFiles = function findFiles(paths, excludes, callback) {
  var basepath = process.cwd(), entries = [];
  ...
  excludes = _.map(excludes || [], function (pattern) {
    // TODO: replace with minimatch
    if (/^\.\.?\//.test(pattern)) {
  ...
  function walk(err) {
    ...
    FsTools.walk(pathname, function (filename, stats, next) {
      entries.push(path.resolve(basepath, filename));
      next();
    }, walk);
  }
