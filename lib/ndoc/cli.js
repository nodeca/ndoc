...
let cli = module.exports = new ArgumentParser({
  version:  require('./version'),
  addHelp:  true,
  formatterClass(options) {
...
cli.findFiles = function findFiles(paths, excludes) {
  ...
  excludes = _.map(excludes || [], p =>
    (_.isFunction(p) ? { test: p } : minimatch.makeRe(p)));

  // return false when file should be ckipped
  function should_exclude(filename) {
    return !_.any(excludes, excl => excl.test(filename));
  }
  ...
  paths.forEach(path => {
    FsTools.walkSync(path, should_exclude, filename => { entries.push(filename); });
  });
