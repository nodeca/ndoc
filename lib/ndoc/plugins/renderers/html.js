...
function marked_highlight(code, lang) {
...
function render_html(ndoc, options) {
  ...
  const templatePath = path.join(__dirname, 'html/templates/layout.jade');
  ...
  // render HTML
  jade.render(fs.readFileSync(templatePath, 'utf8'), vars, (e, val) => {
    if (e) err = e;
    html = val;
  });
